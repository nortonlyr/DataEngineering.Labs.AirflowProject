...
t1e = BashOperator(
    ...
    # bash_command='jupyter nbconvert --to notebook --execute {}'.format(path_abb_nb) --output nyc_abb_vis_aws{{ ds }}.ipynb',
    bash_command='export TMPDIR=/home/ec2-user/airflow/jupyter_nb; jupyter nbconvert --to notebook --execute /home/ec2-user/airflow/jupyter_nb/nyc_abb_vis_aws.ipynb --output nyc_abb_vis_aws_output.ipynb --ExecutePreprocessor.kernel_name=python3',
    dag=dag,
)
...
#########################################################
#AWS load files
hook_s3 = airflow.hooks.S3_hook.S3Hook('aws_norton_airflow')
bucket_name = 'norton.bucket'
path_abb_nb_out = os.path.join(os.path.dirname(os.path.dirname(__file__)),'jupyter_nb/nyc_abb_vis_output.ipynb')

def files_to_s3():
    """
    Save all the ETL files to AWS S3
    """
    hook_s3.load_file(path_etl_abb, 'data/abb_nyc_housing_2019_data2.csv', 'norton.bucket', replace=True)
    hook_s3.load_file(path_etl_park, 'data/nyc_park_data2.csv', 'norton.bucket', replace=True)
    hook_s3.load_file(path_etl_shooting, 'data/nyc_shooting_data2.csv', 'norton.bucket', replace=True)
    hook_s3.load_file(path_etl_hotel, 'data/nyc_hotel_data2.csv', 'norton.bucket', replace=True)
    hook_s3.load_file(path_etl_housing, 'data/nyc_housing_data2.csv', 'norton.bucket', replace=True)
    hook_s3.load_file(path_hotspot, 'data/nyc_hot_spot2.csv', 'norton.bucket', replace=True)

    hook_s3.load_file(path_abb_nb, 'data/nyc_abb_vis_aws.ipynb', 'norton.bucket', replace=True)
    hook_s3.load_file(path_abb_nb_out, 'data/nyc_abb_vis_aws_output.ipynb', 'norton.bucket', replace=True)
    #directory = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data/')


t_files_to_s3 = PythonOperator(
    task_id='files_to_s3',
    python_callable=files_to_s3,
    provide_context=False,
    dag=dag
)
...
#DAGS Flowing Chart
t0 >> t1a >> t1b >> t1c >> t_final >> t1e >> t_files_to_s3
